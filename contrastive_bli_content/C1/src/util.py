...
import io
def load_embs(path, topk = 200_000, dimension = None):
    print(topk)
    print("Loading embeddings")
    # with codecs.open(path, encoding='utf8', errors='ignore', newline='\n') as f:
    with io.open(path, encoding='utf8', errors='ignore', newline='\n') as f:
        ...
        for i, line in enumerate(f):
            ...
            try:
              ...
              vocab_dict[tokens[0]] = i
              v = np.array(tokens[1:], dtype=np.float32)
              embeddings[i, :] = v
            except:
              print(line)
              raise Exception()
    ...
    print("%d word vectors loaded from %d" % (len(vocab_dict), n))
