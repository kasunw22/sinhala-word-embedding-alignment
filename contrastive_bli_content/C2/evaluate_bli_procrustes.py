...
def compute_csls_accuracy(x_src, x_tgt, lexicon, lexicon_size=-1, k=10, bsz=256, top_k=1):
    ...
    # nn = torch.argmax(similarities, dim=1).cpu().tolist()
    # nn = torch.argsort(similarities, dim=1).T[-top_k:].T.cpu().tolist() # argsort in GPU - 24GB GPU Memory not enough
...
if __name__ == '__main__':
    ...
    print("Evaluate BLI")
    str2lang = {"hr":"croatian", "en":"english","fi":"finnish","fr":"french","de":"german","it":"italian","ru":"russian","tr":"turkish", "si": "sinhala", "es": "spanish", "ta": "tamil", "ja": "japanese", "zh": "chinese"}
    ...
    train_path = os.path.join(args.root, "{}2{}_train.txt".format(args.l1,args.l2))
    ...
    for i in tqdm(np.arange(0, len(words_src), bsz)):
        ...
        elif agg_mode == "pooler":
            np_feature_mean_tok1 = outputs_1.pooler_output.detach().cpu()
    ...
    for i in tqdm(np.arange(0, len(words_tgt), bsz)):
        ...
        if agg_mode == "mean_pool":
            ...
            np_feature_mean_tok2 = outputs_2.pooler_output.detach().cpu()
    ...
    for top_k in args.top_k:
        print(f"[INFO] Evaluating top-{top_k} retrieval accuracy...")
        accuracy_BLI = valid_BLI(mix_1, mix_2, src2tgt, lexicon_size_s2t, tgt2src, lexicon_size_t2s, top_k=top_k)
