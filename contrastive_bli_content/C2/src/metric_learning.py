...
class mBERT_pairwise(nn.Module):
    ...
    def forward(self,query_toks1, query_toks2):
        ...
        elif self.agg_mode == "pooler":
            query_embed1 = outputs1.pooler_output
            query_embed2 = outputs2.pooler_output
...
class C2_Metric_Learning(nn.Module):
    ...
    def forward(self, query_toks1, query_toks2):
        # print(f"query_toks1: {query_toks1.shape} -- query_toks2: {query_toks2.shape}----")
        neg_size = self.num_neg
        batch_size = self.batch_size
        N = (neg_size + 1) * batch_size
        query_embed1, query_embed2 = self.model_pairwise(query_toks1, query_toks2)
        
        if len(query_embed1) > N:
          query_embed1 = query_embed1[:N]
        elif len(query_embed1) < N:
          r = int(np.ceil(N / len(query_embed1)))
          query_embed1 = query_embed1.repeat([r, 1])[:N]

        if len(query_embed2) > N:
          query_embed2 = query_embed2[:N]
        elif len(query_embed2) < N:
          r = int(np.ceil(N / len(query_embed2)))
          query_embed2 = query_embed2.repeat([r, 1])[:N]
        ...
        # print(f"[Before] query_embed1: {query_embed1.shape} -- query_embed2: {query_embed2.shape}")
        ...
        # query_embed1 = query_embed1.contiguous().view(-1, neg_size + 1, emb_size)
        # query_embed2 = query_embed2.contiguous().view(-1, neg_size + 1, emb_size)
        # query_embed1 = query_embed1.contiguous().view(batch_size, -1, emb_size)
        # query_embed2 = query_embed2.contiguous().view(batch_size, -1, emb_size)
        # print(f"[After] query_embed1: {query_embed1.shape} -- query_embed2: {query_embed2.shape}")
